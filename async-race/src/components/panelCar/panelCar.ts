...
import { deleteCar, startCar, startEngine } from '../../services/api';
import { DELETE_CAR, statusCar } from '../../store/constants';
import { selectCurrentPage } from '../../store/selector';
import store from '../../store/store';
import { Car } from '../../store/types';
import './style.css';

function createPanelCar(car: Car) {
  ...
  panel.innerHTML = `
    <div class='first-box'>
        ...
        <div class='title-car'>
            <h4 class='brand-car brand ${car.id}'>${car.name}</h4>
        </div>
    </div>
    ...
    <div class='car-box'>
        <div class='car-element' data-id=${car.id}>
  ...
  const aBtn = panel.querySelector('.btn-a') as HTMLButtonElement;
  let isErrorCar = false;

  removeBtn.addEventListener('click', () => handeleDeleteCar(car.id));
  selectBtn.addEventListener('click', () => handeleSelectCar(car));
  aBtn.addEventListener('click', () => handelRunCar(car.id, statusCar[0]));
  ...
  function handelRunCar(id: number, status: string) {
    startCar(String(id), status).then((data) => {
      const { velocity, distance } = data;
      const durationCar = distance / velocity / 1000;
      const finishX = document.querySelector('.distance-car')!.getBoundingClientRect().width;

      animatePosition(id, finishX, durationCar);

      startEngine(String(id), statusCar[2]).then((dataEngine) => {
        if (dataEngine.status === 500) {
          isErrorCar = true;
        }
      });
    });
  ...
  function animatePosition(id: number, endX: number, duration: number) {
    const element = document.querySelector(`[data-id="${String(id)}"]`) as HTMLElement;
    let currentX = 0;
    const frameCount = duration * 100;
